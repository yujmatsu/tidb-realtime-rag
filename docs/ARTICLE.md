...
CREATE TABLE products (
  product_id BIGINT PRIMARY KEY AUTO_RANDOM,
  name VARCHAR(128), stock INT, description TEXT,
  emb VECTOR(64),
...
### ⚠️ §2の「ライブ=0%」との整合（ここが本記事で最も誠実にすべき点）
...
### 一方、HTAPの「本来の効きどころ」＝干渉耐性では効いた
集計の絶対速度では差が出ませんでしたが、HTAPの本質は**「書き込み(OLTP)と分析・ベクトル読み(OLAP)が互いに干渉しないこと」**です。そこで主題に直結する形で、**在庫更新を流し続けながらライブRAG検索のレイテンシを測りました**。

![干渉耐性：書き込み負荷下でも検索レイテンシが横ばい](/images/realtime-rag-tidb/fig6_interference.png)
*図6: 在庫更新(0/5/20件/秒)を並行で流しても、ライブRAG検索の p50/p95 はほぼ一定。*

| 並行書き込み | 検索 p50 | 検索 p95 |
|---|---|---|
| 0（無負荷） | 147ms | 171ms |
| 5更新/秒 | 150ms | 169ms |
| 20更新/秒 | 149ms | 170ms |

→ **更新を流し続けても検索レイテンシは劣化しませんでした**＝「更新が流れ続ける中で常に最新を返す」という本記事の主題に直結するHTAPのポジティブな効き方です。
> 正直な注記：本テストの書き込みスループットは控えめ（writerもネットワークRTT律速で計測窓に数十件）。**大負荷での干渉耐性は今後の課題**ですが、「並行更新で読みが詰まらない」傾向は確認できました。
...
- TiDB の実利は **サーバーレスの手軽さ＋1ステートメント統合＋拡張余地**。HTAP は集計の絶対速度では本検証規模で差が出なかったが、**並行更新下でも検索レイテンシが劣化しない（干渉耐性）は確認**（§5）。
...
> 図解プロンプト（Nano Banana用）はリポジトリの `docs/NANOBANANA-PROMPTS.md` に置いています。
