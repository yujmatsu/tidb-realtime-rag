...
*図6: 「貼り合わせ」構成（左, 同期遅延あり）と単一エンジン（右）。「鮮度ゼロ遅延」は点参照の即時性を指す（分析系は§4の鮮度境界）。*
...
> ⚠️ 正直な注記：**本記事の検証環境では mem9 APIキー未取得のため、この部分は未実行**です（クライアントとデモは `src/expagent/mem9.py` / `scripts/rt_demo_mem9.py` に同梱、`MEM9_API_KEY` 設定で動作）。**鮮度の主結果（§2〜4）は mem9 非依存**です。
...
- TiDB の実利は **サーバーレスの手軽さ＋1ステートメント統合＋拡張余地**。HTAP の性能優位は本検証規模では確認できず、誇張しない。
...
- `scripts/rt_freshness.py`（鮮度誤答率）/ `rt_primary.py`（鮮度境界）/ `rt_demo.py`（NL会話）/ `rt_htap_scale.py`（HTAP）/ `rt_charts.py`（図）
